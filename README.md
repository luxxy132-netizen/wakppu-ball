...
필요한 것을 알아서 설치하고 실행합니다.

처음엔 **"Apple은 악성 코드가 없음을 확인할 수 없습니다"** 창이 뜹니다.
여기서 반드시 **`완료`** 를 누르세요. `휴지통으로 이동` 을 누르면 파일이 지워집니다.
압축을 푼 폴더의 `맥에서 먼저 읽으세요.txt` 에 그림 없이도 따라 할 수 있게 적어 뒀습니다.

> **macOS 15(Sequoia)부터는 "우클릭 → 열기" 우회가 없어졌습니다.** 아래 방법을 써야 합니다.

터미널에서 격리 표식을 지우는 것이 가장 확실합니다. 아래 한 줄을 **복사해서 붙여넣으세요.**
다운로드 폴더와 바탕화면을 모두 훑기 때문에 경로를 따로 칠 필요가 없습니다.

```bash
xattr -cr ~/Downloads/왁뿌볼 ~/Desktop/왁뿌볼 2>/dev/null; echo 끝났습니다
```

폴더가 다른 곳에 있으면 `cd ` 뒤에 폴더를 끌어다 놓아 이동한 뒤 `xattr -cr .` 을 실행합니다.

> `xattr: invalid option -- /` 가 나오면 **`-cr` 과 경로 사이에 띄어쓰기가 빠진 것**입니다.
> 드래그로 경로를 채울 때 자주 생깁니다. 그래서 위처럼 붙여넣는 방식을 권합니다.

또는 한 번 열기를 시도해 차단된 뒤,
**시스템 설정 → 개인정보 보호 및 보안** 을 열고 아래로 내리면
`"시작하기.command"은(는) 확인되지 않은 개발자가 배포했기 때문에 차단되었습니다` 옆에
**`그래도 열기`** 버튼이 나타납니다.
